...
function team.isPkmToLvlUsable(i, lvl_cap) return getPokemonHealth(i) > 0 and game.hasPokemonPPLeft(i) and team.isPkmToLvl(i, lvl_cap)
end
--pkm properties
...
function team.getLowestUsablePkmToLvl(level_cap)
    return team._compare(team.getUsablePkmToLvl(level_cap), gen.minLvl)
end
...
function team.getUsablePkmToLvl(level_cap)
    return team._filter(team.getPkm(), team.isPkmToLvlUsable, level_cap)
end
